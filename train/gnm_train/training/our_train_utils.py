...
def train_eval_loop(
    ...
    for epoch in range(current_epoch, current_epoch + epochs):
        ...
        for dataset_type in test_dataloaders:
            ...
            total_eval_loss = test_action_loss
...
def train(
    ...
    num_batches =len(train_action_loader)
    for i, val in enumerate(train_action_loader):
        (
            action_obs_image,
        ...
        action_obs_ims = action_trans_obs_image.to(device)
        action_obs_pose = action_obs_pos.to(device)
        action_goal_pose = action_goal_pos.to(device)
        action_label = action_label.to(device)

        action_pred = model(action_obs_ims, action_obs_pose, action_goal_pose)
...
def evaluate(
    ...
    num_batches = len(eval_action_loader)

    with torch.no_grad():
        for i, val in enumerate(eval_action_loader):
            (
                action_obs_image,
                action_trans_obs_image,
                action_goal_pos,
