...
def calculate_sin_cos_for_goal(goal_pose: torch.Tensor) -> torch.Tensor:
    """
    Calculate sin and cos of the angle

    Args:
        goal_pose (torch.Tensor): Tensor containing x,y,angle
    Returns:
        torch.Tensor: Goal pose with sin and cos of the angle
    """
    assert goal_pose.shape[0] == 3
    angle_repr = torch.zeros_like(goal_pose[1:])  # tensor with 2 zeros
    angle_repr[0] = torch.cos(goal_pose[2])
    angle_repr[1] = torch.sin(goal_pose[2])
    x_y_pos = goal_pose[:2]
    return torch.cat((x_y_pos, angle_repr), dim=0)
